...
- 状态：本地设计/代码核对，不是生产安全签字；与 `SOURCE_SUBSCRIPTION_AND_INGESTION_PLAN.md` v3.0 一致

## 1. 资产与信任边界

受保护资产包括 PostgreSQL 中的 source/grant/checkpoint/article/origin/audit，权利 evidence/terms snapshot，Object Storage raw payload 及其版本/复制/备份，来源凭据与 signed URL，Worker token，游标/checkpoint，浏览器 profile/cookie/HAR/trace/crash dump，日志/快照/导出，删除回执，编辑/发布门禁以及外部平台账号。

信任边界：浏览器到控制面、控制面到 PostgreSQL/Object Storage、DNS resolver/egress proxy、Secret Manager/Broker、容器编排、控制面到 source Worker、Worker/浏览器到外部来源及 subresource、外部供应商、调度器到控制面、主题派生作业到正式语料。来源响应、标题、正文、URL、redirect、cursor 和错误体全部是不可信输入。

攻击者模型包括恶意来源站点、被攻陷的 Worker/Broker/供应商账号、恶意或误操作管理员、供应链依赖和能访问日志/制品/备份的内部主体。
...
| Worker 越权领取/读取 Secret | Partial：kind/capability/protocol/release control 已校验 | Not implemented / Not Accepted | `.dockerignore` 未排除 `.env*`，Worker 镜像复制工作区并可加载 `.env`，render 使用整份 env file，另有 combined/shared-token fallback（`P0A-IMG-01/P0A-SEC-01`） |
| 数据库/job 保存长期明文 | 本地实现 opaque ref/version 和受控 policy snapshot | 未验收 | 不代表浏览器投影、镜像、日志或备份已安全 |
| Broker 明文 Secret custody | Partial：environment provider 与代理路由可本地运行 | Not implemented | Broker 仍在控制面进程/数据库权限边界内；独立 workload、Secret Manager IAM 和短期能力未实现 |
| 浏览器/API/日志脱敏 | In progress | Not Accepted | actor API 仍可返回 credential ref、checkpoint/cursor、任意 config/locator/capabilities 和未脱敏错误；preview 投影尚未全量验证（`P0A-API-03`） |
| 上游 Secret reflection | 直接字符串命中会拒绝 | Defense-in-depth only | 不覆盖 URL 编码、Base64、分片、部分 token 和派生值；不可用作“泄漏为 0”的主要证据 |
| 凭据轮换后的旧结果 | ref/version 固化、current binding 复核、旧提交 `AUTH_REQUIRED/held` | 未验收 | 生产双版本窗口、旧 pod/能力失效与真实上游验收 |
| 带凭据跨域重定向 | 逐跳 URL/DNS/IP 检查；首次跨 origin 后永久剥离 | 未验收 | 需真实攻击域、HTTPS 降级、敏感 query 和代理旁路测试 |
...
| 转载绕独立性 | origin/evidence family/publisher group 的部分门禁 | Experimental | 声明级补证、合格图匹配、人工修正和冻结阈值未完成 |
...
- 云 Secret Manager provider、生产 workload identity/短期能力和真实授权 API 验收；本地已实现 environment provider + 代理执行式 Broker，不得把它冒充生产 Secret Manager；
- 浏览器 Worker 的非 root、只读根文件系统、无宿主挂载、seccomp/AppArmor、资源上限和任务后销毁；
- 外部平台授权、canonical identity、编辑/删除语义、配额与合同；
...
Foundation GA 只由 release manifest 中实际启用的能力决定：RSS/Public JSON 必须关闭浏览器/API 脱敏、生产 egress、适用 workload 最小权限、来源范围删除与真实 E2E。Credentialed JSON/Broker、Browser/Web、OAuth/Agent、公众号或小红书若未列入 manifest 且入口保持 disabled/hidden，其未完成不阻塞 Foundation，但阻止对应能力 Accepted。任何 Secret-capable 路由如果随制品启用、网络可达或持有生产 Secret，则必须先关闭 `P0A-SEC-01`，不得用“未对外承诺”绕过。
...
6. Worker 尝试把凭据用于其他来源或 origin：Broker 以 source/ref/version/target 四重绑定拒绝并写脱敏兑换审计；不会向 Worker 返回 Secret。
...
- Security：RBAC、SSRF、broker、secret scan、容器和网络攻击测试；
