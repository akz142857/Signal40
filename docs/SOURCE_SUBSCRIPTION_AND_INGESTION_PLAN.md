...
| `R-ROUTE-SQL-01` | 路由层 SQL 从不被自动化测试执行：`npm test` 覆盖 `lib/`，而 `app/api/**` 的「路由测试」只 `readFile` 断言源码文本。两条必然失败的 INSERT 因此通过了 lint、tsc 和全部 260 项测试 | 已加 `test/sql-insert-arity.test.ts` 静态守列值一致性，但它拦不住语义错位（数量对、列放错）与绑定数不匹配；在关键写路径具备真实执行的路由级测试前，**不得仅凭 `npm test` 全绿推断某条 API 可用** | Platform + Delivery | `PRODUCT-E2E-01` 前 | 再出现一例「已实现」但路由从未成功执行的写路径，即升级为 P0 并暂停相关状态晋级 |
