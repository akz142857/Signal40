...
import { scheduledMinuteSince } from './schedule.ts';
import { purgeExpiredSourcePayloads } from './source-retention.ts';
import { pruneStaleWorkers } from './workers.ts';
import { stableHash, WorkflowError, type ContentState, type GateResult, type Role } from './workflow.ts';
...
export type BreakerState = Record<string, { failures: number; openedAt: string | null; lastError: string }>;
...
async function resolveAutomationActor(db: SqlDatabase, userId: string | undefined): Promise<Actor | null> {
...
async function previousBreakers(db: SqlDatabase): Promise<BreakerState> {
  const row = await db
    .prepare('SELECT breakers_json FROM automation_runs ORDER BY started_at DESC LIMIT 1')
...
function breakerOpen(breakers: BreakerState, stage: string, now: Date) {
  ...
  if (!state || state.failures < BREAKER_FAILURE_THRESHOLD || !state.openedAt) return false;
  return now.valueOf() - new Date(state.openedAt).valueOf() < BREAKER_COOLDOWN_MS;
}

function recordFailure(breakers: BreakerState, stage: string, message: string, now: Date) {
  const state = breakers[stage] ?? { failures: 0, openedAt: null, lastError: '' };
  const failures = state.failures + 1;
  breakers[stage] = {
    failures,
    openedAt: failures >= BREAKER_FAILURE_THRESHOLD ? now.toISOString() : state.openedAt,
    lastError: message.slice(0, 500),
  };
}

function recordSuccess(breakers: BreakerState, stage: string) {
  if (breakers[stage]) breakers[stage] = { failures: 0, openedAt: null, lastError: '' };
}
...
async function projectHasOpenIncident(db: SqlDatabase, projectId: string) {
  const row = await db.prepare("SELECT id FROM content_incidents WHERE project_id = ? AND status = 'open' LIMIT 1").bind(projectId).first<{ id: string }>();
  return Boolean(row);
}

async function latestQcPassed(db: SqlDatabase, projectId: string) {
  const row = await db.prepare('SELECT status FROM qc_reports WHERE project_id = ? ORDER BY created_at DESC, seq DESC LIMIT 1').bind(projectId).first<{ status: string }>();
  return row?.status === 'passed';
}

async function dailyAutoPublishCount(db: SqlDatabase, policyId: string, now: Date) {
...
async function dailyAutoProjectCount(db: SqlDatabase, policyId: string, now: Date) {
  const dayStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())).toISOString();
  const row = await db
    .prepare("SELECT COUNT(*) AS total FROM content_projects WHERE automation_policy_id = ? AND created_at >= ?")
...
async function projectCostMicros(db: SqlDatabase, projectId: string) {
  const row = await db
    .prepare('SELECT COALESCE(SUM(CASE WHEN cost_micros > 0 THEN cost_micros ELSE estimated_cost_micros END), 0) AS total FROM jobs WHERE project_id = ?')
    .bind(projectId)
    .first<{ total: number }>();
  return Number(row?.total ?? 0);
}
...
async function monthlyRenderCostMicros(db: SqlDatabase, now: Date) {
  ...
  const row = await db
    .prepare("SELECT COALESCE(SUM(CASE WHEN cost_micros > 0 THEN cost_micros ELSE estimated_cost_micros END), 0) AS total FROM jobs WHERE kind IN ('preview', 'render') AND created_at >= ? AND status != 'cancelled'")
    .bind(monthStart)
    .first<{ total: number }>();
  return Number(row?.total ?? 0);
}
...
async function fail(runner: StageRunner, stage: AutomationStage | 'cleanup' | 'notify', error: unknown, detail: { projectId?: string; topicId?: string; policyId?: string } = {}) {
  ...
  if ((runner.breakers[stage]?.failures ?? 0) >= BREAKER_FAILURE_THRESHOLD) {
    await raiseAttentionItem(runner.db, {
      kind: 'breaker_open',
      severity: 'critical',
...
async function runIngestion(runner: StageRunner, allowed: boolean) {
  ...
  for (const source of rows.results) {
    ...
    try {
      ...
      if (job.created) runner.actions.push({ stage: 'ingestion', action: 'ingestion.enqueued', detail: { sourceConfigId: source.id, scheduledMinute, jobId: job.id } });
    } catch (error) {
      await fail(runner, 'ingestion', error);
    }
  }
  // 只有整轮没出错才复位：出过错还复位，等于永远攒不够连续失败次数，熔断形同虚设。
  if (runner.errors.length === errorsBefore) recordSuccess(runner.breakers, 'ingestion');
}

/** 2. 选题质量评估：写入 `topics.quality_json`，不达标的进待办箱。 */
async function runTopicQuality(runner: StageRunner, allowed: boolean, policies: AutomationPolicy[]) {
  if (!allowed) return;
  ...
  const rows = await db
    ...
    `)
    .bind(limits.topics)
    .all<{ id: string; score: number; updated_at: string; quality_json: string }>();
  const minScore = policies.length ? Math.min(...policies.map((policy) => policy.scope.minTopicScore)) : 60;
  for (const row of rows.results) {
    try {
      const topic = await loadTopic(db, row.id);
      if (!topic) continue;
      const quality = assessTopicQuality(topic, now);
      await db.prepare('UPDATE topics SET quality_json = ? WHERE id = ?').bind(JSON.stringify(quality), row.id).run();
      runner.actions.push({ stage: 'topic_quality', action: 'topic.quality_assessed', topicId: row.id, detail: { automatable: quality.automatable, coherence: quality.coherence } });
      // 只有「本来够格进自动化」的选题才值得打扰人：门禁已过、分数达标，却因为质量指标被挡下。
      if (!quality.automatable && topic.gate.passed && topic.score >= minScore) {
  ...
  if (runner.errors.length === errorsBefore) recordSuccess(runner.breakers, 'topic_quality');
}

/** 3. 建项目：达标选题 + 策略允许 + 未超日限额。 */
async function runProjectCreation(runner: StageRunner, policies: AutomationPolicy[]) {
  const { db, now, actor, limits } = runner;
  let created = 0;
  for (const policy of policies) {
    ...
    for (const row of rows.results) {
      ...
      const quality = parseJson<TopicQuality | Record<string, never>>(row.quality_json, {});
      try {
        const topic = await loadTopic(db, row.id);
        if (!topic || topic.verificationStatus !== 'verified' || !topic.gate.passed) continue;
        const sourceTypes = [...new Set(topic.articles.map((article) => article.sourceType))];
        if (!policyMatchesTopic(policy, { score: row.score, sourceTypes, quality: quality as TopicQuality })) continue;
        const project = createProjectV2(topic, now);
        if (!policyMatchesProject(policy, { brand: project.identity.brand, locale: project.identity.locale })) continue;
        const result = await createContentProject(db, project, actor, now);
        if (!result.created) continue;
        await db.prepare('UPDATE content_projects SET automation_policy_id = ? WHERE id = ?').bind(policy.id, result.project.id).run();
        created += 1;
        runner.actions.push({ stage: 'project_creation', action: 'project.created', projectId: result.project.id, topicId: row.id, policyId: policy.id });
      } catch (error) {
...
async function tryAutoApproval(
  ...
  const { db, now } = runner;
  const independentSourceCount = new Set(project.project.research.claims.flatMap((claim) => claim.evidence.map((evidence) => evidence.sourceId))).size;
  const unresolvedConflicts = project.project.research.conflicts.filter((conflict) => !conflict.resolution?.trim()).length;
  ...
  const decision = autoApprovalDecision(policy, kind, {
    ...
    topicQualityOk: Boolean(parseJson<{ automatable?: boolean }>(quality?.quality_json, {}).automatable),
  ...
  const authorizedId = kind === 'publish' ? policy.publishAuthorizedBy : policy.researchAuthorizedBy;
  ...
  runner.actions.push({ stage: 'advance', action: `approval.auto_${kind}`, projectId: project.id, policyId: policy.id, detail: { actorId: member.id } });
...
async function enqueueStageJobs(runner: StageRunner, policy: AutomationPolicy, project: ProjectRecord): Promise<JobStageOutcome> {
  ...
  if (project.state === 'SCRIPT_APPROVED' && !project.project.audio.objectKey) {
    ...
    const job = await enqueueJob(db, {
      kind: 'voice',
      projectId: project.id,
      payload: { projectId: project.id, scriptVersion: project.project.script.version, scriptHash },
      idempotencyKey: `voice:${project.id}:${scriptHash}`,
      actor,
      trigger: 'automation',
      policyId: policy.id,
    }, now);
    if (job.created) runner.actions.push({ stage: 'jobs', action: 'voice.enqueued', projectId: project.id, policyId: policy.id, detail: { jobId: job.id } });
  ...
  if (project.state === 'ASSETS_READY') {
    ...
    if (perProjectLimit > 0 && await projectCostMicros(db, project.id) >= perProjectLimit) {
      await raiseAttentionItem(db, {
        kind: 'budget_exceeded',
        severity: 'warning',
        projectId: project.id,
    ...
    const job = await enqueueJob(db, {
      kind: 'render',
      projectId: project.id,
      payload: { projectId: project.id, snapshotHash: project.project.render.snapshotHash, compositionId: project.project.render.compositionId },
      idempotencyKey: `render:${project.id}:${project.project.render.snapshotHash}`,
      actor,
      trigger: 'automation',
      policyId: policy.id,
    }, now);
    if (job.created) {
      await db
        .prepare('INSERT INTO render_snapshots (id, project_id, snapshot_json, snapshot_hash, template_id, template_version, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING')
        .bind(`render_${crypto.randomUUID()}`, project.id, JSON.stringify(project.project), project.project.render.snapshotHash, project.project.render.templateId, project.project.render.templateVersion, actor.id, now.toISOString())
        .run();
      runner.actions.push({ stage: 'jobs', action: 'render.enqueued', projectId: project.id, policyId: policy.id, detail: { jobId: job.id } });
    }
    return job.created ? 'enqueued' : 'exists';
  }
  return 'skipped';
...
async function runPublish(runner: StageRunner, policy: AutomationPolicy, project: ProjectRecord) {
  const { db, now, actor } = runner;
  ...
  const channel = project.project.distribution.channelPreset === 'youtube' ? 'youtube' : 'package';
  ...
  runner.actions.push({ stage: 'publish', action: 'publish.scheduled', projectId: project.id, policyId: policy.id, detail: { channel } });
  return true;
}

/** 4. 推进一个项目：门禁已过且在自动范围内的转换，一轮最多 stepsPerProject 步。 */
async function advanceProject(runner: StageRunner, policy: AutomationPolicy, projectId: string) {
  const { db, now, actor, limits } = runner;
  for (let step = 0; step < limits.stepsPerProject; step += 1) {
    ...
    if (await projectHasOpenIncident(db, projectId)) {
      await pauseProjectAutomation(db, projectId, '项目存在未关闭的内容事件，自动化已停止。');
      await raiseAttentionItem(db, {
        kind: 'incident_open',
        severity: 'critical',
        projectId,
      ...
      runner.actions.push({ stage: 'advance', action: 'automation.paused_by_incident', projectId, policyId: policy.id });
      return;
    }
    ...
    if (project.state === 'ASSETS_READY' && jobOutcome !== 'enqueued' && jobOutcome !== 'exists') return;
    ...
    const conflicting = gate(gates, 'G2_AUTO_EVIDENCE')?.reasons.filter((reason) => reason.includes('反驳证据')) ?? [];
    ...
    if (approvalKind) {
      const gateCode = target === 'EDITOR_APPROVED' ? 'G3_MANUAL_RESEARCH' : target === 'SCRIPT_APPROVED' ? 'G4_SCRIPT_COVERAGE' : target === 'QC_APPROVED' ? 'G6_CONTENT_TECH_QC' : 'G7_PUBLISH_APPROVAL';
      if (!gate(gates, gateCode)?.passed) {
        const approval = await tryAutoApproval(runner, policy, project, gates, approvalKind);
        if (!approval.approved) return;
      }
    }
    const freshGates = approvalKind ? await evaluateProjectGates(db, projectId) : gates;
    ...
    try {
      ...
      runner.actions.push({ stage: 'advance', action: 'project.transitioned', projectId, policyId: policy.id, detail: { from: project.state, to: target } });
    } catch (error) {
      if (error instanceof WorkflowError && error.code === 'GATE_FAILED') {
        ...
        await raiseAttentionItem(db, {
          ...
          detail: { blocking: blocking.map((item) => ({ code: item.code, reasons: item.reasons })) },
        }, now);
        return;
      }
      throw error;
  ...
  const rows = await db
    .prepare(`
      ...
      WHERE pj.status = 'published'
        AND NOT EXISTS (SELECT 1 FROM metric_snapshots ms WHERE ms.publish_job_id = pj.id)
      ORDER BY pj.updated_at DESC LIMIT 50
    `)
    .all<{ id: string; project_id: string; updated_at: string }>();
  for (const row of rows.results) {
    const ageHours = (now.valueOf() - new Date(row.updated_at).valueOf()) / 3_600_000;
    const window = ageHours >= 168 ? '7d' : ageHours >= 24 ? '24h' : ageHours >= 2 ? '2h' : null;
    if (!window) continue;
    await raiseAttentionItem(db, {
      kind: 'metrics_due',
      severity: 'info',
    ...
    runner.actions.push({ stage: 'metrics', action: 'metrics.reminder', projectId: row.project_id, detail: { publishJobId: row.id, window } });
  }
...
async function runCleanup(runner: StageRunner) {
  ...
  try {
    ...
    if (workers.deleted) runner.actions.push({ stage: 'cleanup', action: 'workers.pruned', detail: workers });
    if (context.storage) {
      const retention = await purgeExpiredSourcePayloads(db, context.storage, now);
      if (retention.deletedObjects) runner.actions.push({ stage: 'cleanup', action: 'source_payloads.purged', detail: retention });
    }
  } catch (error) {
...
async function raiseDeadLetterItems(runner: StageRunner) {
  const { db, now } = runner;
  const rows = await db
    .prepare("SELECT id, kind, project_id, last_error, updated_at FROM jobs WHERE status = 'dead_letter' ORDER BY updated_at DESC LIMIT 50")
    .all<{ id: string; kind: string; project_id: string | null; last_error: string | null; updated_at: string }>();
  for (const row of rows.results) {
 ...
 */
export async function runAutomationTick(context: OrchestratorContext): Promise<AutomationTickResult> {
  const db = context.db;
  ...
  const finish = async (status: AutomationTickResult['status'], projectCount: number, skippedReason?: string) => {
    ...
    await db
      .prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
      .bind(runId, trigger, status, startedAt, result.finishedAt, result.durationMs, projectCount, JSON.stringify(actions), JSON.stringify(breakers), JSON.stringify(skippedReason ? [...errors, { stage: 'tick', message: skippedReason }] : errors))
      .run();
    return result;
  };
  ...
  if (!actor) {
    await raiseAttentionItem(db, {
      ...
      reason: '未配置有效的自动化服务账号（SIGNAL40_AUTOMATION_ACTOR_ID 指向的成员必须存在、active 且角色为 admin），编排引擎不做任何写入。',
    }, now).catch(() => undefined);
    return finish('skipped', 0, 'automation_actor_missing');
  }

  const runner: StageRunner = { db, now, actor, limits, actions, errors, breakers, context };
  ...
  try {
    candidateIds = await db.transaction(async (tx) => {
      const lock = await tx.prepare('SELECT pg_try_advisory_xact_lock(?) AS locked').bind(AUTOMATION_ADVISORY_LOCK_KEY).first<{ locked: boolean }>();
  ...
  if (candidateIds === null) return finish('skipped', 0, 'another_tick_running');

  const stageAllowed = (stage: AutomationStage) => (policies.length ? policies.some((policy) => stageMode(policy, stage) === 'auto') : stage === 'ingestion' || stage === 'topic_quality' || stage === 'metrics');

  if (!breakerOpen(breakers, 'ingestion', now)) await runIngestion(runner, stageAllowed('ingestion'));
  if (!breakerOpen(breakers, 'topic_quality', now)) await runTopicQuality(runner, stageAllowed('topic_quality'), policies);
  if (!breakerOpen(breakers, 'project_creation', now)) await runProjectCreation(runner, policies);
  ...
  if (!breakerOpen(breakers, 'advance', now)) {
    for (const projectId of candidateIds) {
      const project = await loadContentProject(db, projectId);
      if (!project) continue;
      const policy = policies.find((candidate) => (project.automationPolicyId ? candidate.id === project.automationPolicyId : policyMatchesProject(candidate, project)));
      if (!policy) continue;
      handledProjects += 1;
      try {
        await advanceProject(runner, policy, projectId);
        recordSuccess(breakers, 'advance');
      } catch (error) {
        await fail(runner, 'advance', error, { projectId, policyId: policy.id });
      }
    }
  }

  if (!breakerOpen(breakers, 'metrics', now)) await runMetrics(runner, stageAllowed('metrics'));
  await raiseDeadLetterItems(runner);
  await runCleanup(runner);

  const notified = await notifyPendingAttention(db, { url: context.notify?.url, secret: context.notify?.secret, limit: limits.notifications, fetchImpl: context.fetchImpl }, now);
  if ('sent' in notified && notified.sent) actions.push({ stage: 'notify', action: 'attention.notified', detail: notified });
