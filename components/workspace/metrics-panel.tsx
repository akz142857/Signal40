...
export function MetricsPanel({ project, onSaved, onMessage }: { project: ProjectRecord; onSaved: () => Promise<void>; onMessage: (message: string) => void }) {
  ...
  return <section className="rounded-2xl border border-border bg-card p-5">
    <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-secondary"><BarChart3 className="size-5" /></span><div><p className="font-mono text-xs uppercase tracking-[0.15em] text-muted-foreground">Post-publish learning</p><h2 className="text-lg font-semibold tracking-tight">2h / 24h / 7d 指标回流</h2></div></div>
