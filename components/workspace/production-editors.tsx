...
export function ScriptEditor({ project, onSaved, onMessage }: { project: ProjectRecord; onSaved: () => Promise<void>; onMessage: (value: string) => void }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const response = await fetch(`/api/v1/projects/${project.id}/scripts`, { method: 'POST', headers: { 'content-type': 'application/json', 'if-match': `"${project.version}"`, 'x-signal-role': 'editor', 'x-signal-actor-id': 'local-editor' }, body: JSON.stringify({ script: { ...script, humanModifiedBy: 'local-editor', humanModifiedAt: new Date().toISOString() } }) });
...
export function StoryboardEditor({ project, onSaved, onMessage }: { project: ProjectRecord; onSaved: () => Promise<void>; onMessage: (value: string) => void }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const response = await fetch(`/api/v1/projects/${project.id}/storyboards`, { method: 'POST', headers: { 'content-type': 'application/json', 'if-match': `"${project.version}"`, 'x-signal-role': 'producer' }, body: JSON.stringify({ timeline }) });
      if (!response.ok) throw new Error(await errorMessage(response));
      await onSaved();
