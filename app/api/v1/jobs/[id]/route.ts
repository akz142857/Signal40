...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const job = await db.prepare('SELECT id, kind, project_id, status, attempt, max_attempts, available_at, lease_owner, lease_expires_at, result_json, last_error, cost_micros, created_at, updated_at FROM jobs WHERE id = ? LIMIT 1').bind(id).first();
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  const prior = await db.prepare("SELECT id FROM audit_events WHERE entity_type = 'job' AND entity_id = ? AND metadata_json ->> 'idempotencyKey' = ? LIMIT 1").bind(id, key).first();
  if (prior) {
    const current = await db.prepare('SELECT id, status FROM jobs WHERE id = ? LIMIT 1').bind(id).first();
    return Response.json({ job: current, replayed: true });
  }
  const job = await db.prepare('SELECT id, kind, project_id, status, payload_json FROM jobs WHERE id = ? LIMIT 1').bind(id).first<{ id: string; kind: string; project_id: string | null; status: string; payload_json: unknown }>();
  if (!job) return Response.json({ error: '作业不存在。' }, { status: 404 });
  ...
  const statements: SqlStatement[] = [
    db.prepare('UPDATE jobs SET status = ?, attempt = CASE WHEN ? THEN 0 ELSE attempt END, available_at = ?, lease_owner = NULL, lease_expires_at = NULL, last_error = CASE WHEN ? THEN NULL ELSE last_error END, updated_at = ? WHERE id = ? AND status = ?').bind(nextStatus, replay ? 1 : 0, now, replay ? 1 : 0, now, id, job.status),
  ...
  if (job.kind === 'ingestion') {
    const payload = (job.payload_json && typeof job.payload_json === 'object' ? job.payload_json : {}) as { ingestionRunId?: string };
    if (payload.ingestionRunId) statements.push(db.prepare('UPDATE ingestion_runs SET status = ?, finished_at = ? WHERE id = ?').bind(replay ? 'queued' : 'cancelled', replay ? null : now, payload.ingestionRunId));
  }
  if (job.kind === 'render' && job.project_id && !replay) statements.push(db.prepare("UPDATE content_projects SET state = 'CHANGES_REQUESTED', version = version + 1, updated_at = ? WHERE id = ? AND state IN ('RENDER_QUEUED', 'RENDERING')").bind(now, job.project_id));
