...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const publishJob = await db.prepare('SELECT id, project_id, channel, status FROM publish_jobs WHERE id = ? LIMIT 1').bind(id).first<{ id: string; project_id: string; channel: string; status: string }>();
  ...
  const job = body.jobId ? await db.prepare("SELECT id FROM jobs WHERE id = ? AND kind = 'publish' AND status = 'leased' AND project_id = ?").bind(body.jobId, publishJob.project_id).first() : null;
  ...
  const statements: SqlStatement[] = [
    db.prepare("UPDATE publish_jobs SET status = 'published', external_id = ?, package_object_key = ?, final_url = ?, platform_response_json = ?, updated_at = ? WHERE id = ? AND status = 'publishing'").bind(body.externalId ?? null, packageObjectKey, body.finalUrl ?? null, body.platformResponse === undefined ? null : JSON.stringify(body.platformResponse), now, id),
