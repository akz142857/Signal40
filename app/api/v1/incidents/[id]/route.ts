...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const incident = await env.DB.prepare("SELECT id, project_id, status FROM content_incidents WHERE id = ? LIMIT 1").bind(id).first<{ id: string; project_id: string; status: string }>();
  ...
  await env.DB.batch([
    env.DB.prepare("UPDATE content_incidents SET status = 'resolved', resolution = ?, updated_at = ? WHERE id = ? AND status = 'open'").bind(body.resolution.trim(), now, id),
