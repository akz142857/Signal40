...
import { pauseAutomationStatement } from '@/lib/control-plane';
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const incidents = await db.prepare('SELECT id, publish_job_id, kind, severity, status, reason, resolution, actor_id, created_at, updated_at FROM content_incidents WHERE project_id = ? ORDER BY created_at DESC').bind(id).all();
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  const project = await db.prepare('SELECT id FROM content_projects WHERE id = ? LIMIT 1').bind(id).first();
  if (!project) return Response.json({ error: '项目不存在。' }, { status: 404 });
  if (body.publishJobId) {
    const publish = await db.prepare('SELECT id FROM publish_jobs WHERE id = ? AND project_id = ? LIMIT 1').bind(body.publishJobId, id).first();
