...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [job, asset] = await Promise.all([
    db.prepare("SELECT id FROM jobs WHERE id = ? AND project_id = ? AND kind = 'voice' AND status = 'leased'").bind(body.jobId, id).first(),
  ...
  const results = await db.batch([
    db.prepare('UPDATE content_projects SET project_json = ?, immutable_hash = ?, version = version + 1, updated_at = ? WHERE id = ? AND version = ?').bind(JSON.stringify(nextProject), stableHash(nextProject), now, id, project.version),
    db.prepare('INSERT INTO voice_tracks (id, project_id, script_version, provider, voice, object_key, duration_ms, speed_milli, pronunciation_json, audio_sha256, fallback_provider, cost_micros, alignment_json, created_at) SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? WHERE EXISTS (SELECT 1 FROM content_projects WHERE id = ? AND version = ? AND updated_at = ?)').bind(voiceTrackId, id, body.scriptVersion, body.provider, body.voice, asset.object_key, Number(body.durationMs), Math.round(nextProject.audio.speed * 1000), JSON.stringify(nextProject.audio.pronunciationDictionary), asset.sha256, nextProject.audio.fallbackProvider, nextProject.audio.estimatedCostMicros, JSON.stringify(body.alignment), now, id, project.version + 1, now),
    db.prepare("INSERT INTO caption_tracks (id, project_id, voice_track_id, format, content, created_at) SELECT ?, ?, ?, 'json', ?, ? WHERE EXISTS (SELECT 1 FROM content_projects WHERE id = ? AND version = ? AND updated_at = ?)").bind(captionTrackId, id, voiceTrackId, JSON.stringify(body.captions), now, id, project.version + 1, now),
