...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await env.DB.prepare('SELECT pea.*, e.name, e.primary_metric FROM project_experiment_assignments pea JOIN experiments e ON e.id = pea.experiment_id WHERE pea.project_id = ? ORDER BY pea.assigned_at DESC').bind(id).all();
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  const experiment = body.experimentId ? await env.DB.prepare("SELECT id, variants_json, allocation_bps_json FROM experiments WHERE id = ? AND status = 'running' LIMIT 1").bind(body.experimentId).first<{ id: string; variants_json: string; allocation_bps_json: string }>() : null;
  if (!experiment) return Response.json({ error: '实验不存在或未运行。' }, { status: 409 });
  const existing = await env.DB.prepare('SELECT variant, assignment_hash FROM project_experiment_assignments WHERE project_id = ? AND experiment_id = ? LIMIT 1').bind(id, experiment.id).first();
  ...
  await env.DB.prepare('INSERT INTO project_experiment_assignments (id, project_id, experiment_id, variant, assignment_hash, assigned_at) VALUES (?, ?, ?, ?, ?, ?)').bind(assignmentId, id, experiment.id, variant, assignmentHash, new Date().toISOString()).run();
