...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await env.DB.prepare('SELECT id, publish_job_id, captured_at, metrics_json, attribution_json FROM metric_snapshots WHERE project_id = ? ORDER BY captured_at ASC').bind(id).all();
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const publishJob = await env.DB.prepare("SELECT id FROM publish_jobs WHERE id = ? AND project_id = ? AND status = 'published'").bind(body.publishJobId, id).first();
  if (!publishJob) return Response.json({ error: '发布任务不存在或尚未发布。' }, { status: 409 });
  const existing = await env.DB.prepare('SELECT id FROM metric_snapshots WHERE project_id = ? AND idempotency_key = ?').bind(id, key).first<{ id: string }>();
  if (existing) return Response.json({ snapshot: { id: existing.id }, replayed: true });
  const snapshotId = `metric_${crypto.randomUUID()}`;
  const now = new Date().toISOString();
  const assignments = await env.DB.prepare('SELECT experiment_id, variant, assignment_hash FROM project_experiment_assignments WHERE project_id = ?').bind(id).all();
  ...
  await env.DB.batch([
    env.DB.prepare('INSERT INTO metric_snapshots (id, project_id, publish_job_id, idempotency_key, captured_at, metrics_json, attribution_json) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(snapshotId, id, body.publishJobId, key, capturedAt.toISOString(), JSON.stringify(body.metrics), JSON.stringify(attribution)),
    env.DB.prepare("UPDATE content_projects SET state = 'MEASURED', version = version + 1, updated_at = ? WHERE id = ? AND state = 'PUBLISHED'").bind(now, id),
