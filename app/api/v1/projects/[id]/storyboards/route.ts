...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const versions = await db.prepare('SELECT id, version, storyboard_json, content_hash, created_by, created_at FROM storyboard_versions WHERE project_id = ? ORDER BY version DESC').bind(id).all();
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  const result = await saveProjectSection(db, { projectId: id, expectedVersion, section: 'storyboard', value: body.timeline, actor });
