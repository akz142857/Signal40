...
export async function POST(request: Request) {
  ...
  const asset = await env.DB.prepare('SELECT id FROM assets WHERE object_key = ? LIMIT 1').bind(body.objectKey).first();
  ...
  const secret = env.MEDIA_SIGNING_SECRET || (new URL(request.url).hostname === 'localhost' || new URL(request.url).hostname === '127.0.0.1' ? 'signal40-local-media-signing-key' : '');
