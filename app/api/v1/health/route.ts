...
export async function GET() {
  ...
  try {
    const [database, queue] = await Promise.all([
      ...
      env.DB.prepare("SELECT SUM(CASE WHEN status IN ('queued', 'retrying', 'leased') THEN 1 ELSE 0 END) AS active, SUM(CASE WHEN status = 'dead_letter' THEN 1 ELSE 0 END) AS dead_letter FROM jobs").first<{ active: number | null; dead_letter: number | null }>(),
    ]);
