...
export async function GET(request: Request) {
  ...
  const result = await env.DB.prepare('SELECT * FROM calibration_runs ORDER BY created_at DESC LIMIT 200').all();
...
export async function POST(request: Request) {
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO calibration_runs (id, algorithm_version, dataset_label, case_count, metrics_json, status, created_by, note, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'candidate', ?, ?, ?, ?)").bind(id, body.algorithmVersion.trim(), body.datasetLabel.trim(), body.caseCount, JSON.stringify(body.metrics), actor.id, body.note?.trim() ?? '', now, now),
