...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const run = await env.DB.prepare("SELECT id, status, created_by FROM calibration_runs WHERE id = ? LIMIT 1").bind(id).first<{ id: string; status: string; created_by: string }>();
  ...
  await env.DB.batch([
    env.DB.prepare('UPDATE calibration_runs SET status = ?, approved_by = ?, note = ?, updated_at = ? WHERE id = ? AND status = \'candidate\'').bind(body.decision, actor.id, body.note.trim(), now, id),
