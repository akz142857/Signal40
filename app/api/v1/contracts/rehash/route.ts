...
export async function POST(request: Request) {
  ...
  const rows = await env.DB.prepare('SELECT id, state, version, project_json, immutable_hash FROM content_projects ORDER BY id LIMIT 500').all<{ id: string; state: string; version: number; project_json: string; immutable_hash: string }>();
  ...
  for (const row of rows.results) {
    ...
    const results = await env.DB.batch([
      env.DB.prepare('UPDATE content_projects SET project_json = ?, immutable_hash = ?, state = ?, version = version + 1, updated_at = ? WHERE id = ? AND version = ?').bind(JSON.stringify(project), nextHash, nextState, now, row.id, row.version),
