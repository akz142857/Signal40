...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const session = await env.DB.prepare("SELECT * FROM asset_upload_sessions WHERE id = ? AND status = 'open'").bind(id).first<Record<string, unknown>>();
  ...
  const upload = env.MEDIA.resumeMultipartUpload(String(session.object_key), String(session.upload_id));
  const object = await upload.complete(body.parts);
  const assetId = `asset_${crypto.randomUUID()}`;
  const sha256 = object.checksums.sha256 ? [...new Uint8Array(object.checksums.sha256)].map((value) => value.toString(16).padStart(2, '0')).join('') : object.etag;
  const asset = {
    id: assetId,
  ...
  try {
    const results = await env.DB.batch([
      env.DB.prepare('UPDATE content_projects SET project_json = ?, immutable_hash = ?, state = ?, version = version + 1, updated_at = ? WHERE id = ? AND version = ?').bind(JSON.stringify(nextProject), stableHash(nextProject), project.state === 'CHANGES_REQUESTED' ? 'SCRIPT_APPROVED' : project.state, now, projectId, project.version),
