...
export async function PUT(request: Request, context: { params: Promise<{ id: string; partNumber: string }> }) {
  ...
  const session = await env.DB.prepare("SELECT upload_id, object_key, expires_at FROM asset_upload_sessions WHERE id = ? AND status = 'open'").bind(id).first<{ upload_id: string; object_key: string; expires_at: string }>();
  ...
  const part = await env.MEDIA.resumeMultipartUpload(session.object_key, session.upload_id).uploadPart(partNumber, data);
