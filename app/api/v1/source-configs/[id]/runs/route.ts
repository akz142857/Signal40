...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.prepare('SELECT * FROM ingestion_runs WHERE source_config_id = ? ORDER BY created_at DESC LIMIT 100').bind(id).all();
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  const source = await db.prepare('SELECT id, enabled, rights_status, checkpoint, rate_limit_per_minute FROM source_configs WHERE id = ?').bind(id).first<{ id: string; enabled: number; rights_status: string; checkpoint: string | null; rate_limit_per_minute: number }>();
  ...
  const replay = await db.prepare("SELECT id FROM jobs WHERE kind = 'ingestion' AND idempotency_key = ? LIMIT 1").bind(key).first<{ id: string }>();
  if (!replay) {
    const since = new Date(Date.now() - 60_000).toISOString();
    const recent = await db.prepare('SELECT created_at FROM ingestion_runs WHERE source_config_id = ? AND created_at > ? ORDER BY created_at ASC LIMIT 600').bind(id, since).all<{ created_at: string }>();
