...
import { disconnectSource } from '@/lib/source-lifecycle';
import { boundedTrimmedText, positiveInteger } from '@/lib/source-action-validation';

export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const actor = await resolveRequestActor(request);
  if (!actor || actor.role !== 'admin') return sourceApiError('只有管理员可以断开来源连接。', 403);
  ...
  const result = await disconnectSource(db, { sourceId: id, expectedVersion, reason, actor });
