...
export async function GET(request: Request) {
  ...
  const result = await env.DB.prepare('SELECT id, name, adapter, config_json, rights_status, rate_limit_per_minute, retention_mode, retention_days, enabled, version, schedule_cron, checkpoint, last_success_at, last_error, created_at, updated_at FROM source_configs ORDER BY name').all();
...
export async function POST(request: Request) {
  ...
  const existing = await env.DB.prepare("SELECT id FROM audit_events WHERE action = 'source.created' AND json_extract(metadata_json, '$.idempotencyKey') = ? LIMIT 1").bind(idempotencyKey).first<{ id: string }>();
  ...
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO source_configs (id, name, adapter, config_json, rights_status, rate_limit_per_minute, retention_mode, retention_days, enabled, version, schedule_cron, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?, ?, ?)`).bind(id, input.name.trim(), input.adapter, JSON.stringify(config), input.rightsStatus, rateLimitPerMinute, retention.mode, retention.days, input.scheduleCron ?? null, now, now),
    env.DB.prepare(`INSERT INTO audit_events (id, actor_id, actor_role, action, entity_type, entity_id, after_hash, metadata_json, request_id, created_at) VALUES (?, ?, ?, 'source.created', 'source_config', ?, ?, ?, ?, ?)`).bind(`audit_${crypto.randomUUID()}`, actor.id, actor.role, id, stableHash(input), JSON.stringify({ idempotencyKey }), crypto.randomUUID(), now),
  ]);
  const scheduled = input.scheduleCron ? await enqueueIngestionRun(env.DB, { sourceConfigId: id, idempotencyKey: `initial:${id}`, actor }) : null;
