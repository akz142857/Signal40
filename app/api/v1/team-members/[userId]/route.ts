...
export async function PATCH(request: Request, context: { params: Promise<{ userId: string }> }) {
  ...
  const current = await db.prepare('SELECT user_id, email, role, status FROM team_members WHERE user_id = ? LIMIT 1').bind(userId).first<{ user_id: string; email: string; role: string; status: string }>();
  ...
  if (current.role === 'admin' && current.status === 'active' && (next.role !== 'admin' || next.status !== 'active')) {
    const count = await db.prepare("SELECT COUNT(*) AS total FROM team_members WHERE role = 'admin' AND status = 'active'").first<{ total: number }>();
    if (Number(count?.total ?? 0) <= 1) return Response.json({ error: '不能停用或降级最后一名管理员。' }, { status: 409 });
  }
  const now = new Date().toISOString();
  await db.batch([
    db.prepare('UPDATE team_members SET role = ?, status = ?, updated_at = ? WHERE user_id = ?').bind(next.role, next.status, now, userId),
