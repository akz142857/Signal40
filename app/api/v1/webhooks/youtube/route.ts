...
export async function POST(request: Request) {
  ...
  if (!eventId || eventId.length > 200 || !env.WEBHOOK_SECRET) return Response.json({ error: 'Webhook 未配置或缺少事件 ID。' }, { status: 401 });
  if (!(await verifyWebhookSignature({ body: raw, timestamp, signature, secret: env.WEBHOOK_SECRET }))) return Response.json({ error: 'Webhook 签名无效或已超出 5 分钟时间窗。' }, { status: 401 });
  const existing = await env.DB.prepare("SELECT id, status FROM webhook_events WHERE provider = 'youtube' AND external_event_id = ? LIMIT 1").bind(eventId).first<{ id: string; status: string }>();
  ...
  const publish = await env.DB.prepare("SELECT id, project_id FROM publish_jobs WHERE id = ? AND channel = 'youtube' LIMIT 1").bind(body.publishJobId).first<{ id: string; project_id: string }>();
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO webhook_events (id, provider, external_event_id, signature_hash, payload_hash, status, occurred_at, payload_json, received_at) VALUES (?, 'youtube', ?, ?, ?, 'processed', ?, ?, ?)").bind(webhookId, eventId, stableHash(signature), webhookPayloadHash(raw), occurredAt, raw, now),
    env.DB.prepare('UPDATE publish_jobs SET status = ?, external_id = COALESCE(?, external_id), updated_at = ? WHERE id = ?').bind(body.status, body.externalId ?? null, now, body.publishJobId),
