...
export async function POST(request: Request) {
  ...
  const existing = await db.prepare("SELECT entity_id FROM audit_events WHERE action = 'experiment.created' AND metadata_json ->> 'idempotencyKey' = ? LIMIT 1").bind(key).first<{ entity_id: string }>();
  ...
  await db.batch([
    ...
    db.prepare("INSERT INTO audit_events (id, actor_id, actor_role, action, entity_type, entity_id, after_hash, metadata_json, request_id, created_at) VALUES (?, ?, ?, 'experiment.created', 'experiment', ?, ?, ?, ?, ?)").bind(`audit_${crypto.randomUUID()}`, actor.id, actor.role, id, stableHash(body), JSON.stringify({ idempotencyKey: key }), crypto.randomUUID(), now),
  ]);
