...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const experiment = await db.prepare('SELECT id, status FROM experiments WHERE id = ? LIMIT 1').bind(id).first<{ id: string; status: string }>();
  ...
  await db.batch([
    db.prepare('UPDATE experiments SET status = ?, starts_at = CASE WHEN ? = \'running\' THEN COALESCE(starts_at, ?) ELSE starts_at END, ends_at = CASE WHEN ? IN (\'completed\', \'cancelled\') THEN ? ELSE ends_at END, updated_at = ? WHERE id = ? AND status = ?').bind(body.status, body.status, now, body.status, now, now, id, experiment.status),
