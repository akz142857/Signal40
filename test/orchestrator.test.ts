...
async function seedProject(
  ...
  const actor = {
    id: serviceActorId,
    email: 'automation@signal40.test',
    role: 'admin' as const,
  };
  const created = await createContentProject(db, project, actor, baseTime);
  await db.client.query(
    'UPDATE content_projects SET state = $1, automation_policy_id = $2 WHERE id = $3',
    [state, 'policy_test', created.project.id],
  );
...
async function tick(
  ...
  return runAutomationTick({
    db,
    now,
    trigger: 'scheduler',
    automationActorId: serviceActorId,
    ...overrides,
  });
}
...
void test('没有配置服务账号时引擎不做任何写入', async () => {
  ...
  const items = await db.client.query(
    "SELECT kind FROM attention_items WHERE status = 'open'",
  );
  assert.equal(
    (items.rows[0] as { kind: string }).kind,
...
void test('门禁未过时不推进，只往待办箱写一条原因', async () => {
  ...
  const items = await db.client.query(
    'SELECT kind, reason FROM attention_items WHERE project_id = $1',
    [projectId],
  );
  assert.equal(
    (items.rows[0] as { kind: string }).kind,
...
void test('tick 重入不会重复入队配音作业', async () => {
  ...
  const jobs = await db.client.query(
    "SELECT COUNT(*) AS total FROM jobs WHERE project_id = $1 AND kind = 'voice'",
    [projectId],
  );
...
void test('预先授权过期后策略失效，项目转由人工推进', async () => {
  ...
  await tick(db, at(10));
  assert.equal(
    (await loadContentProject(db, projectId))?.state,
    'SCRIPT_DRAFT',
  );
  const approved = await db.client.query(
    "SELECT actor_id FROM approvals WHERE project_id = $1 AND kind = 'research'",
    [projectId],
  );
  assert.equal(
    (approved.rows[0] as { actor_id: string }).actor_id,
    'editor-1',
    '自动放行写入的必须是策略里那个真人',
  );

  await db.client.query(
    "UPDATE content_projects SET state = 'EVIDENCE_READY' WHERE id = $1",
    [projectId],
  );
  await db.client.query('DELETE FROM approvals WHERE project_id = $1', [
    projectId,
  ]);
  // 过期之后同一条策略不再被选中，状态原地不动。
  const later = await tick(db, at(600));
  const expiredProject = await loadContentProject(db, projectId);
  assert.equal(expiredProject?.state, 'EVIDENCE_READY');
  assert.equal(expiredProject?.automationMode, 'manual');
  assert.match(expiredProject?.automationPausedReason ?? '', /授权已过期/);
  const expiredPolicy = await db.client.query(
    'SELECT enabled FROM automation_policies WHERE id = $1',
    ['policy_test'],
  );
  assert.equal(
    Number((expiredPolicy.rows[0] as { enabled: number }).enabled),
    0,
  );
  assert.equal(
    later.actions.some((action) => action.action === 'approval.auto_research'),
    false,
  );
});

void test('全局暂停会跳过全部编排阶段并保留项目状态', async () => {
  const db = await createMemoryPg();
  await seedMembers(db);
  await seedPolicy(db);
  const projectId = await seedProject(db, 'SCRIPT_APPROVED');
  await db.client.query(
    "UPDATE automation_control SET paused = 1, reason = '维护窗口', updated_at = $1 WHERE id = 'global'",
    [baseTime.toISOString()],
  );
  const result = await tick(db, at(1));
  assert.equal(result.status, 'skipped');
  assert.equal(result.skippedReason, 'global_pause:维护窗口');
  assert.equal(
    (await loadContentProject(db, projectId))?.state,
    'SCRIPT_APPROVED',
  );
  const jobs = await db.client.query('SELECT COUNT(*) AS total FROM jobs');
  assert.equal(Number((jobs.rows[0] as { total: number }).total), 0);
});

void test('单轮 jobs 上限会限制采集作业实际入队数', async () => {
  const db = await createMemoryPg();
  await seedMembers(db);
  await seedPolicy(db);
  await db.client.query(
    `INSERT INTO workers (id, hostname, kinds_json, capabilities_json, version, last_heartbeat_at, created_at)
    VALUES ('ingestion-worker', 'test', '["ingestion"]', '["source:rss"]', 'test', $1, $1)`,
    [at(0).toISOString()],
  );
  for (const id of ['source_a', 'source_b']) {
    await db.client.query(
      `INSERT INTO source_configs
      (id, name, adapter, config_json, config_hash, rights_status, schedule_cron, enabled,
       lifecycle_status, next_run_at, created_at, updated_at)
      VALUES ($1, $1, 'rss', '{}', $4, 'approved', '* * * * *', 1, 'enabled', $2, $3, $3)`,
      [id, at(0).toISOString(), at(-120).toISOString(), `hash-${id}`],
    );
    await db.client.query(
      ...
      VALUES ($1, $2, 'test', 'rss', '["title","summary","url","publishedAt","author"]',
        'finance-editorial-ingestion', 'normalized-metadata', 'test', 'v1', 'test',
        $3, $3, 1, $4, $3)`,
      [`rights_${id}`, id, at(-120).toISOString(), `hash-${id}`],
    );
  }
  await tick(db, at(1), { limits: { jobs: 1 } });
  const jobs = await db.client.query(
    "SELECT COUNT(*) AS total FROM jobs WHERE kind = 'ingestion'",
  );
  assert.equal(Number((jobs.rows[0] as { total: number }).total), 1);
});

void test('来源月预算耗尽时调度跳过 occurrence、推进下次时间并留下待办', async () => {
  const db = await createMemoryPg();
  await seedMembers(db);
  await seedPolicy(db);
  await db.client.query(
    `
    INSERT INTO workers
      (id, hostname, kinds_json, capabilities_json, version, last_heartbeat_at, created_at)
    VALUES ('budget-worker', 'test', '["ingestion"]', '["source:rss"]', 'test', $1, $1)
  `,
    [baseTime.toISOString()],
  );
  await db.client.query(
    ...
    INSERT INTO source_configs
      (id, name, adapter, platform, config_json, config_hash, rights_status,
       schedule_cron, enabled, lifecycle_status, next_run_at,
       cost_micros_per_request, estimated_requests_per_run, monthly_budget_micros,
       created_at, updated_at)
    VALUES ('source-budget-tick', 'Budget tick', 'rss', 'rss', '{}',
      'hash-budget-tick', 'approved', '* * * * *', 1, 'enabled', $1,
      1000, 1, 1000, $2, $2)
  `,
    [at(0).toISOString(), at(-120).toISOString()],
  );
  await db.client.query(
    `
    INSERT INTO source_rights_grants
      (id, source_config_id, principal, provider, permitted_fields_json, purpose,
       usage_scope, evidence_ref, terms_version, verified_by, granted_at,
       verified_at, source_version, config_hash, created_at)
    VALUES ('rights-budget-tick', 'source-budget-tick', 'test', 'rss',
      '["title","summary","url","publishedAt","author"]',
      'finance-editorial-ingestion', 'normalized-metadata', 'test', 'v1',
      'test', $1, $1, 1, 'hash-budget-tick', $1)
  `,
    [at(-120).toISOString()],
  );
  await db.client.query(
    ...
    VALUES ('spent-budget-tick', 'source-budget-tick', 'succeeded', 1000, $1)
  `,
    [baseTime.toISOString()],
  );
  ...
  assert.equal(
    result.actions.some(
      (action) => action.action === 'ingestion.budget_blocked',
    ),
    true,
  );
  const source = await db.client.query(
    "SELECT next_run_at FROM source_configs WHERE id = 'source-budget-tick'",
  );
  assert.equal(
    (source.rows[0] as { next_run_at: string }).next_run_at,
    at(60).toISOString(),
  );
  const jobs = await db.client.query(
    "SELECT COUNT(*) AS total FROM jobs WHERE idempotency_key LIKE 'schedule:source-budget-tick:%'",
  );
  assert.equal(Number((jobs.rows[0] as { total: number }).total), 0);
  const attention = await db.client.query(
    "SELECT severity FROM attention_items WHERE kind = 'source_budget'",
  );
  assert.equal(
...
void test('预算软阈值按来源优先级降频并逐 occurrence 留下可重放审计', async () => {
  const db = await createMemoryPg();
  await seedMembers(db);
  await seedPolicy(db);
  await db.client.query(
    `INSERT INTO workers
      (id, hostname, kinds_json, capabilities_json, version, last_heartbeat_at, created_at)
     VALUES ('throttle-worker', 'test', '["ingestion"]', '["source:rss"]', 'test', $1, $1)`,
    [baseTime.toISOString()],
  );
  await db.client.query(
    `INSERT INTO source_configs
      (id, name, adapter, platform, config_json, config_hash, rights_status,
       schedule_cron, enabled, lifecycle_status, next_run_at,
       cost_micros_per_request, estimated_requests_per_run, monthly_budget_micros,
       budget_soft_limit_percent, schedule_priority, auto_throttle_enabled,
       created_at, updated_at)
     VALUES ('source-throttle-tick', 'Throttle tick', 'rss', 'rss', '{}',
       'hash-throttle-tick', 'approved', '* * * * *', 1, 'enabled', $1,
       1000, 1, 10000, 80, 20, 1, $2, $2)`,
    [at(0).toISOString(), at(-120).toISOString()],
  );
  await db.client.query(
    `INSERT INTO source_rights_grants
      (id, source_config_id, principal, provider, permitted_fields_json, purpose,
       usage_scope, evidence_ref, terms_version, verified_by, granted_at,
       verified_at, source_version, config_hash, created_at)
     VALUES ('rights-throttle-tick', 'source-throttle-tick', 'test', 'rss',
       '["title","summary","url","publishedAt","author"]',
       'finance-editorial-ingestion', 'normalized-metadata', 'test', 'v1',
       'test', $1, $1, 1, 'hash-throttle-tick', $1)`,
    [at(-120).toISOString()],
  );
  await db.client.query(
    `INSERT INTO ingestion_runs
      (id, source_config_id, status, cost_micros, created_at)
     VALUES ('spent-throttle-tick', 'source-throttle-tick', 'succeeded', 7000, $1)`,
    [baseTime.toISOString()],
  );

  const first = await tick(db, at(1));
  const second = await tick(db, at(1));
  assert.equal(
    first.actions.some(
      (action) => action.action === 'ingestion.schedule_throttled',
    ),
    true,
  );
  assert.equal(
  ...
  const source = await db.client.query(
    `SELECT next_run_at, schedule_cron, effective_schedule_multiplier,
       schedule_throttle_reason, schedule_throttle_recovery_at
     FROM source_configs WHERE id = 'source-throttle-tick'`,
  );
  assert.deepEqual(source.rows[0], {
    next_run_at: at(4 * 60).toISOString(),
    schedule_cron: '* * * * *',
    effective_schedule_multiplier: 4,
    schedule_throttle_reason: 'budget_soft_limit',
    schedule_throttle_recovery_at: '2026-10-01T00:00:00.000Z',
  });
  const throttles = await db.client.query(
    `SELECT scheduled_for FROM source_schedule_throttles
     WHERE source_config_id = 'source-throttle-tick' ORDER BY scheduled_for`,
  );
  assert.deepEqual(
    throttles.rows.map((row) => (row as { scheduled_for: string }).scheduled_for),
    [at(60).toISOString(), at(120).toISOString(), at(180).toISOString()],
  );
  const audits = await db.client.query(
    `SELECT COUNT(*) AS total FROM audit_events
     WHERE entity_id = 'source-throttle-tick'
       AND action = 'source.schedule_throttle_activated'`,
  );
  assert.equal(Number((audits.rows[0] as { total: number }).total), 1);
});

void test('指标回流分别追踪 2h、24h、7d 窗口，不会被早期快照永久屏蔽', async () => {
  const db = await createMemoryPg();
  await seedMembers(db);
  await seedPolicy(db);
  const projectId = await seedProject(db, 'PUBLISHED');
  await db.client.query(
    `INSERT INTO publish_jobs (id, project_id, channel, logical_key, status, account_id, title, description, tags_json, updated_at, created_at)
    VALUES ('publish_metrics', $1, 'package', 'metrics-key', 'published', 'default', '测试', '', '[]', $2, $2)`,
    [projectId, at(-8 * 24 * 3600).toISOString()],
  );
  await db.client.query(
    `INSERT INTO metric_snapshots (id, project_id, publish_job_id, idempotency_key, captured_at, metrics_json, attribution_json)
    VALUES ('metric_2h', $1, 'publish_metrics', '2h', $2, '{}', '{"source":"manual","window":"2h"}')`,
    [projectId, at(-7 * 24 * 3600).toISOString()],
  );
  await tick(db, at(1));
  const items = await db.client.query(
    "SELECT detail_json::jsonb ->> 'window' AS window FROM attention_items WHERE kind = 'metrics_due' ORDER BY detail_json::jsonb ->> 'window'",
  );
  assert.deepEqual(
    items.rows.map((row) => (row as { window: string }).window),
    ['24h', '7d'],
  );
});
...
void test('授权人相同的策略即使绕过校验落库，自动放行也会被拒绝', async () => {
  ...
  const items = await db.client.query(
    "SELECT reason FROM attention_items WHERE project_id = $1 AND kind = 'auto_approval_rejected'",
    [projectId],
  );
  assert.ok(
...
void test('月度渲染预算耗尽时不入队渲染作业', async () => {
  ...
  const jobs = await db.client.query(
    "SELECT COUNT(*) AS total FROM jobs WHERE project_id = $1 AND kind = 'render'",
    [projectId],
  );
  assert.equal(Number((jobs.rows[0] as { total: number }).total), 0);
  const items = await db.client.query(
    "SELECT kind FROM attention_items WHERE kind = 'budget_exceeded'",
  );
...
void test('连续失败达到阈值后该阶段熔断，后续 tick 不再重试', async () => {
  ...
  assert.equal(
    Number((jobs.rows[0] as { total: number }).total),
    0,
    '熔断期间不该继续入队采集作业',
  );
...
void test('内容事件一开，自动化立即停止并写明原因', async () => {
  ...
  const jobs = await db.client.query(
    'SELECT COUNT(*) AS total FROM jobs WHERE project_id = $1',
    [projectId],
  );
  assert.equal(Number((jobs.rows[0] as { total: number }).total), 0);
});
...
void test('人工审批会把项目踢出自动化，审计区分得出是人还是策略', async () => {
  ...
  const audit = await db.client.query(
    "SELECT metadata_json ->> 'trigger' AS trigger FROM audit_events WHERE action = 'approval.approved' AND project_id = $1",
    [projectId],
  );
...
void test('选题质量不达标时不自动放行研究审批', async () => {
  ...
  assert.match(
    String((items.rows[0] as { reason: string }).reason),
    /选题质量指标未达标/,
  );
});
